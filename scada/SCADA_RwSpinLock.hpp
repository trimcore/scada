#ifndef SCADA_RWSPINLOCK_HPP
#define SCADA_RWSPINLOCK_HPP
...
namespace Scada {
    template <typename> class RwSpinLockScopeShared;
    template <typename> class RwSpinLockScopeExclusive;

    // RwSpinLock
    //  - high performance, cross-process, reader-writter spin lock implementation
    //  - writers don't have priority
    //  - inspired by: https://software.intel.com/en-us/articles/implementing-scalable-atomic-locks-for-multi-core-intel-em64t-and-ia32-architectures/
    //  - StateType - underlying interlocked counter variable
    //     - supported: 'short', 'long' or 'long long'
    //  - TODO: recursive exclusive locking
    //     - new format: shared lock: lock count, like now (upper bit 0)
    //                   exclusive: lock counter (1-255), 24-bit thread ID shifted 2 bits right
    //
    template <typename StateType = long>
    class RwSpinLock {
    ...
    public:
        ...
        [[nodiscard]] inline bool TryAcquireShared () noexcept {
            auto s = *static_cast <volatile StateType *> (&this->state);
